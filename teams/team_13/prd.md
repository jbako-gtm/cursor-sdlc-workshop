...
| Alex D  | Win % display        | *(add username)*   | Shows the selected team’s % chance of winning (big, clear number + team name)     |
| David C | "Why this %"         | *(add username)*   | Short breakdown or factors (e.g. form, draw, history) — all stub copy             |
| Chris D | Team rankings        | *(add username)*   | All teams ranked by win % (table or list from stub data)                          |
| Sam G   | Theme or celebration | *(add username)*   | Light/dark/World Cup theme toggle, or confetti/celebration when you pick a team   |
